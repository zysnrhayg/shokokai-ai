...
function finishExpertImport(formCode, themeCode) {
  const detailGridEl = document.querySelector('.mi-detail-grid');
  if (detailGridEl) detailGridEl.classList.remove('mi-detail-grid--import-pending');

  if (detailGridEl) detailGridEl.classList.remove('mi-detail-grid--swapped');
  const voiceCardEl = document.getElementById('mi-voice-card');
  const expertPanelsEl = document.getElementById('mi-expert-import-panels');
  const importStepsDropdownEl = document.getElementById('mi-import-steps-dropdown');
  const utilityLabelEl = document.getElementById('mi-utility-label');
  ...
  const SAMPLE_VALUES = {
    'G-4': {
      business: '株式会社北海道フーズ', industry: 'E', date: '2026-08-10',
      timeStart: '13:00', timeEnd: '14:30',
      content: '専門家派遣により省力化機械の導入について助言を実施。人手不足解消に向けた投資計画を提案した。',
    },
    'G-5': {
      business: '有限会社さっぽろ工務店', industry: 'D', date: '2026-08-09',
      timeStart: '10:00', timeEnd: '11:00',
      content: '個人相談会にて、インボイス制度対応（適格請求書発行事業者登録）について相談を受けた。',
    },
  };
  const sample = SAMPLE_VALUES[formCode] || SAMPLE_VALUES['G-4'];
  ...
  const jigyoshoEl = document.getElementById('mi-jigyosho');
  if (jigyoshoEl) { jigyoshoEl.value = sample.business; jigyoshoEl.dispatchEvent(new Event('input')); }

  const jigyoshoDropdown = document.getElementById('mi-jigyosho-dropdown');
  if (jigyoshoDropdown) jigyoshoDropdown.classList.remove('is-open');
  const industryEl = document.getElementById('mi-industry');
  if (industryEl) {
    industryEl.value = sample.industry;
    industryEl.dispatchEvent(new Event('change'));
    if (typeof SelectWidth !== 'undefined') SelectWidth.fit(industryEl);
    industryEl.classList.toggle('is-placeholder', industryEl.value === '');
  }
  const dateEl = document.getElementById('mi-date');
  if (dateEl) { dateEl.value = sample.date; dateEl.dispatchEvent(new Event('input')); }
  const timeStartEl = document.getElementById('mi-time-start');
  if (timeStartEl) { timeStartEl.value = sample.timeStart; timeStartEl.dispatchEvent(new Event('input')); }
  const timeEndEl = document.getElementById('mi-time-end');
  if (timeEndEl) { timeEndEl.value = sample.timeEnd; timeEndEl.dispatchEvent(new Event('input')); }
  document.querySelectorAll('.mi-theme-checkbox').forEach((cb) => {
    cb.checked = (cb.value === themeCode);
    cb.dispatchEvent(new Event('change'));
  });
  const contentEl = document.getElementById('mi-content');
  if (contentEl) { contentEl.value = sample.content; contentEl.dispatchEvent(new Event('input')); }
}
