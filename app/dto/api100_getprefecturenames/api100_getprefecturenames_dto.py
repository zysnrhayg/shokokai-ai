#py_entity.vm make dto templete
from utils.base_entity import BaseEntity
from voluptuous import Schema, Required, Range,MultipleInvalid,Length,Match,ALLOW_EXTRA,All
import re
from  utils.datecheck_hiduke import DateCheckHiduke
from  utils.datecheck_hiduke_ny import DateCheckHidukeNeGaTu
from  utils.datecheck_hiduke_yd import DateCheckHidukeGeTuNiChi
from  utils.datecheck_hiduke_compare import DateCheckHidukeCompare
from  utils.range_check import RangeCheck
class Api100GetprefecturenamesDto(BaseEntity):
		
	def __init__(self,mode,actflg,triggerid,row,prefecturecode,name,shortname,region,sortorder,ispseudo):
		super().__init__(mode,actflg,triggerid,row)
			#prefecture_code
		self.prefecturecode = prefecturecode
			#name
		self.name = name
			#short_name
		self.shortname = shortname
			#region
		self.region = region
			#sort_order
		self.sortorder = sortorder
			#is_pseudo
		self.ispseudo = ispseudo
	
	def dict_to_json(dict):
		return Api100GetprefecturenamesDto(dict.get("mode",""),dict.get("actflg",""),dict.get("triggerid",""),dict.get("row",""),dict.get("prefecturecode",""),dict.get("name",""),dict.get("shortname",""),dict.get("region",""),dict.get("sortorder",""),dict.get("ispseudo",""))
	
	
	
