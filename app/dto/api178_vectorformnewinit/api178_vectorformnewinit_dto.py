...
class Api178VectorformnewinitDto(BaseEntity):
		
	def __init__(self,mode,actflg,triggerid,row,result):
		super().__init__(mode,actflg,triggerid,row)
			#result
		self.result = result
	
	def dict_to_json(dict):
		return Api178VectorformnewinitDto(dict.get("mode",""),dict.get("actflg",""),dict.get("triggerid",""),dict.get("row",""),dict.get("result",""))
