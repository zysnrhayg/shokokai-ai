...
class M001Dto(BaseEntity):
		
	def __init__(self,mode,actflg,triggerid,row,kaishaid):
		super().__init__(mode,actflg,triggerid,row)
			#会社ID
		self.kaishaid = kaishaid
	
	def dict_to_json(dict):
		return M001Dto(dict.get("mode",""),dict.get("actflg",""),dict.get("triggerid",""),dict.get("row",""),dict.get("kaishaid",""))
