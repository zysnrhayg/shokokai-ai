...
class AccountdeleteapiService:
    ...
    def accountdeleteapi(self, accountdeleteapi_dto, jsonObj):
        USER_ACCOUNT_ID = utils.string_util.changeNullToBlank(accountdeleteapi_dto.useraccountid)
        deleted_by = utils.string_util.changeNullToBlank(session.get("USER_ACCOUNT_ID", ""))
        utils.config.global_log.debug(str(threading.current_thread().native_id) + ": start")
        try:
            if deleted_by and USER_ACCOUNT_ID and str(USER_ACCOUNT_ID) == str(deleted_by):
                jsonObj.setValue(utils.json_constant.JSONID_ERR, "自分自身のアカウントは削除できません")
